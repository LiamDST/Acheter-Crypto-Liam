...
DO $$
BEGIN
  -- Check if the trigger for modules already exists
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'update_modules_updated_at'
  ) THEN
    EXECUTE 'CREATE TRIGGER update_modules_updated_at
      BEFORE UPDATE ON modules
      FOR EACH ROW
      EXECUTE FUNCTION update_updated_at_column()';
  END IF;

  -- Check if the trigger for module_content already exists
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'update_module_content_updated_at'
  ) THEN
    EXECUTE 'CREATE TRIGGER update_module_content_updated_at
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    AND policyname = 'Authenticated users can view free modules'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'modules'
    AND policyname = 'Users with active subscriptions can view all modules'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'modules'
    AND policyname = 'Service role can manage all modules'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'module_content'
    AND policyname = 'Authenticated users can view content of free modules'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'module_content'
    AND policyname = 'Users with active subscriptions can view all module content'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'module_content'
    AND policyname = 'Service role can manage all module content'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'modules' 
    AND indexname = 'idx_modules_is_free'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'modules' 
    AND indexname = 'idx_modules_order'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'module_content' 
    AND indexname = 'idx_module_content_module_id'
  ) THEN
...
-- Insert sample data for module 1 (free) if it doesn't exist
...
-- Insert sample data for other modules (premium) if they don't exist
INSERT INTO modules (id, title, description, "order", is_free)
VALUES 
